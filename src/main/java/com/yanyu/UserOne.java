...
public class UserOne {
    private static Logger LOGGER = LoggerFactory.getLogger(UserOne.class);
    ...
    private final String appAccount = "UserOne";
    private MIMCUser user;

    public UserOne() throws Exception {
        /**
    ...
    private void init(final MIMCUser mimcUser) throws Exception {
        mimcUser.registerTokenFetcher(new UserOne.MIMCCaseTokenFetcher(appId, appKey, appSecurity, url, mimcUser.getAppAccount()));
...
//            }
            for (int i = 0 ; i < 1000; i++) {
                user.sendMessage("UserTwo", "hello world-" + i);
                Thread.sleep(1000);
            }
            while (true) {
                String msg = sc.nextLine();
