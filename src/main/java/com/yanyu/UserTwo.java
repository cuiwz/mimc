...
public class UserTwo {
    ...
    private static Logger LOGGER = LoggerFactory.getLogger(UserTwo.class);
    ...
    private final String appAccount = "UserTwo";
    private MIMCUser user;

    public UserTwo() throws Exception {
        /**
    ...
    private void init(final MIMCUser mimcUser) throws Exception {
        mimcUser.registerTokenFetcher(new UserTwo.MIMCCaseTokenFetcher(appId, appKey, appSecurity, url, mimcUser.getAppAccount()));
            ...
            public boolean handleMessage(List<MIMCMessage> packets) {
                for (MIMCMessage p : packets) {
                    try {
                        ...
                        LOGGER.info("ReceiveMessage, P2P, {}-->{}, packetId:{}, payload:{}, count:{}",
                                p.getFromAccount(), p.getToAccount(), p.getPacketId(), new String(msg.getContent()), count.get());
                    } catch (Exception e) {
